# AI Consensus System - M2

This document outlines the technical architecture and data flow of the Multi-Model Orchestration Pipeline (M2). The system is designed as a **Sequential Multi-Agent Consensus** engine, where multiple LLMs compete, refine, and undergo **randomized blind peer evaluation** to reach a high-quality "Winning" output with improved cost efficiency.
...
**Generate Random Model Mapping** $\Rightarrow$ **Build Custom Scoring Prompt Per Model** $\Rightarrow$ **Each Model Evaluates One Random Alternative** $\Rightarrow$ **Score Output Captured & Structured** $\Rightarrow$
...
- **Output Captured & Structured:**
  - Raw consensus responses saved to:
    ...
    ```
    outputs_m2/stage_02_consensus/
    ```

  - Structured consensus results saved to:

    ```
    outputs_m2/stage_02_consensus_structured/
    ```
...
### **STAGE 3 | Randomized Blind Scoring Round**

- **Input:** All refined outputs from Stage 2.

- **Logic:**
  1. Generates a random model-to-model evaluation mapping using `generate_random_model_mapping()`.
  2. Each model is assigned exactly one alternative model’s output for evaluation.
  3. Builds customized scoring prompts using `build_scoring_prompt_orchestrator()`.
  4. Executes scoring using `run_models_with_custom_prompts()`.

- **Scoring Structure:**
  - Each model evaluates exactly **one** randomly assigned alternative answer.
  - Each model’s answer is evaluated by exactly **one** alternative model.
  - This forms a randomized 1-to-1 evaluation cycle.

- **Score Output Captured & Structured:**
  - Random model mapping saved to:

    ```
    outputs_m2/stage_03_model_mapping/
    ```

  - Raw scoring responses saved to:

    ```
    outputs_m2/stage_03_scoring/
    ```

  - Structured scoring results saved to:

    ```
    outputs_m2/stage_03_scoring_structured/
    ```

- **Purpose:**
  Reduces scoring prompt size and cost while maintaining fairness through randomization and blind evaluation.

> **🔍 STAGE 3.5 | Reload Scoring Round Output (Optional/Debug)**
>
> - Reloads raw or structured scoring results.
> - Enables inspection of individual judge behavior and mapping integrity.
...
1. **Parse:**
   ...
   - Saved to:
     ...
     ```
     outputs_m2/stage_04_raw_scores/
     ```
...
2. **Aggregation:**
   ...
   - Saved to:
     ...
     ```
     outputs_m2/stage_04_aggregated_scores/
     ```
...
3. **Internal Weightage System:**
   ...
   - Weighted results saved to:
     ...
     ```
     outputs_m2/stage_04_weighted_scores/
     ```
...
4. **Token Summary Generation:**
   ...
   - Saved to:
     ...
     ```
     outputs_m2/stage_05_token_summary/
     ```
...
6. **CSV Export:**
   - `export_csv()` saves reports to:
     ...
     ```
     outputs_m2/stage_05_token_report_stage/
     outputs_m2/stage_05_token_report_model_summary/
     ```

- **Result:** Fully logged, cost-optimized, auditable AI decision pipeline with randomized blind peer evaluation.
...
Unlike a standard memory-only script, this architecture saves the state to the file system at every stage (`outputs_m2/stage_xx_...`). This allows for:
...
### **2. Randomized Blind Peer Evaluation**

M2 introduces randomized one-to-one evaluation:

- Each model evaluates exactly one alternative.
- Each model is evaluated exactly once.
- Reduces scoring prompt size significantly.
- Improves scalability for large model pools.

---

### **3. Internal Weightage System with Bias Control**

The winner is selected through:

- Randomized blind scoring
- Self-bias removal (`remove_self_bias=True`)
- Weighted score calculation

This mitigates structural bias while keeping scoring cost-efficient.
