...
def run_ai_consensus_system_m2():
    try:
        # =========================
        ...
        initial_log_messages = [f"AI_CONSENSUS_SYSTEM_M2 | START | AI_CONSENSUS_SYSTEM_M2_V1"]
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_0}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | SUCCESS | Start Timestamp captured", current_time=start_timestamp)
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_0}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_1}")
        ...
        initial_response_log = save_response_log(
            ...
            dir="outputs_m2/stage_01_initial",
        )
        ...
        initial_structured_result = format_structured_response(
            ...
            dir="outputs_m2/stage_01_initial_structured",
        )

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_1}")
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_1v}")
        ...
        # import json
        # with open(f"outputs_m2/stage_01_initial/{request_id}.json", "r") as file:
        ...
        # import json
        # with open(f"outputs_m2/stage_01_initial_structured/{request_id}.json", "r") as file:
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_1v}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_2}")
        ...
        consensus_response_log = save_response_log(
            ...
            dir="outputs_m2/stage_02_consensus",
        )
        ...
        consensus_structured_result = format_structured_response(
            ...
            dir="outputs_m2/stage_02_consensus_structured",
        )

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_2}")
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_2v}")

        # # Load previously saved consensus round responses (useful for replay/debug)
        # import json
        # with open(f"outputs_m2/stage_02_consensus/{request_id}.json", "r") as file:
        ...
        # import json
        # with open(f"outputs_m2/stage_02_consensus_structured/{request_id}.json", "r") as file:
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_2v}")


        # ==========================================
        # STAGE - 3 : Random Mapping & Scoring Round
        # ==========================================

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_3}")

        # Generate random model mapping
        from functions.random_model_mapper import generate_random_model_mapping
        random_map = generate_random_model_mapping(  
            request_id=request_id,
            model_outputs=consensus_structured_result,
            directory="outputs_m2/stage_03_model_mapping"
        )

        # Build scoring prompt for that specific answer
        ...
        from functions.ai_custom_prompt_orchestrator import run_models_with_custom_prompts
        scoring_model_outputs = run_models_with_custom_prompts(
        ...
        scoring_response_log = save_response_log(
            request_id=request_id,
            prompt=prompts_with_model,
            outputs=scoring_model_outputs,
            dir="outputs_m2/stage_03_scoring",
        )
        ...
        scoring_structured_result = format_structured_response(
            ...
            dir="outputs_m2/stage_03_scoring_structured",
        )

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_3}")
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_3v}")

        # # Load previously saved raw scoring responses (useful for replay/debug)
        # import json
        # with open(f"outputs_m2/stage_03_scoring/{request_id}.json", "r") as file:
        ...
        # import json
        # with open(f"outputs_m2/stage_03_scoring_structured/{request_id}.json", "r") as file:
        ...
        # # Updating log entry 
        # write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_3v}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_4}")
        ...
        save_scores_log(
            ...
            dir="outputs_m2/stage_04_raw_scores",
        )
        ...
        save_scores_log(
            ...
            dir="outputs_m2/stage_04_aggregated_scores"
        )
        ...
        save_scores_log(
            ...
            dir="outputs_m2/stage_04_weighted_scores"
        )
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_4}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | START | {STAGE_5}")
        ...
        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | SUCCESS | End Timestamp captured", current_time=end_timestamp)

        # Calculate total pipeline execution time (in seconds)
        total_execution_time = round(end_perf_counter - start_perf_counter, 4)

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | SUCCESS | Total pipeline execution time {total_execution_time} seconds")
        ...
        save_winner_log(
            ...
            dir="outputs_m2/stage_05_winner"
        )
        ...
        token_summary = generate_token_summary(
            ...
            directory="outputs_m2/stage_05_token_summary"
        )
        ...
        export_csv(
            ...
            directory="outputs_m2/stage_05_token_report_stage"
        )
        ...
        export_csv(
            ...
            directory="outputs_m2/stage_05_token_report_model_summary"
        )

        # Updating log entry 
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | END | {STAGE_5}")
        ...
        # Log fatal error
        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | FAILED | Error in main.py file")
        write_log(filename=request_id, message=f"ERROR : {str(e)}")

        write_log(filename=request_id, message=f"AI_CONSENSUS_SYSTEM_M2 | TRACEBACK | Tracebacking error")
